...
const REQUIRED_CLASSIC_THEME_FILES = [
  "style.css",
  "functions.php",
  "index.php",
  "header.php",
...
export async function runTheme(ctx: MigrationContext, bus: UiBus): Promise<void> {
  bus.pushStreamEvent("theme", { type: "phase_start", phase: "theme", message: "weaving the theme — turning source UI into classic PHP templates" });
  ...
  const pagesTable = pages.length
    ? [
        "| slug | title | permalink | source layout | role | required classic template |",
        "|---|---|---|---|---|---|",
        ...pages.map((p) => {
  ...
  const missing = REQUIRED_CLASSIC_THEME_FILES.filter((f) => !existsSync(join(ctx.themeDir, f)));
  if (missing.length > 0) {
...
async function writeThemeFallbacks(
  ...
  await write(
    "style.css",
    `/*\nTheme Name: ${name}\nDescription: Generated by to-wordpress (fallback classic theme).\nVersion: 0.1.0\nRequires at least: 6.3\nText Domain: ${detected.themeSlug}\n*/\n`,
  );
  await write(
    "functions.php",
  ...
  await write(
    "index.php",
    "<?php get_header(); ?>\n<main>\n<?php if (have_posts()) : while (have_posts()) : the_post(); ?>\n<article><h2><a href=\"<?php the_permalink(); ?>\"><?php echo wp_kses_post(get_the_title()); ?></a></h2><?php the_excerpt(); ?></article>\n<?php endwhile; endif; ?>\n</main>\n<?php get_footer(); ?>\n",
  );
  await write(
    "single.php",
